...
abstract class Loader
{
	...
	public static function setup($enablePsr = true, $enablePrefixes = true, $enableClasses = true)
	{
		if ($enableClasses)
		{
			...
			spl_autoload_register(array('\\JFusion\\Loader', 'load'));
		}

		if ($enablePrefixes)
		{
//			self::registerPrefix('JFusion', JFUSIONPATH_PLATFORM . '/jfusion');

			// Register the prefix autoloader.
			spl_autoload_register(array('\\JFusion\\Loader', '_autoload'));
		}

		if ($enablePsr)
		{
			// Register the PSR-0 based autoloader.
			spl_autoload_register(array('\\JFusion\\Loader', 'loadByPsr0'));
			spl_autoload_register(array('\\JFusion\\Loader', 'loadByAlias'));
		}
	}
